...
def get_args_parser():
    ...
    parser.add_argument('--mbart_path', type=str, default=None,
                        help='Path to the MBart model (overridden by config).')
    parser.add_argument('--tokenizer_path', type=str, default=None,
                        help='Path to the MBart tokenizer (overridden by config).')
    ...
    parser.add_argument('--text_path', type=str, default="data/labels",
                        help='Path to the text data (overridden by config).')
    ...
    parser.add_argument('--output_dir', type=str, default="outputs", help='Output directory (overridden by config).')
    parser.add_argument('--log_dir', type=str, default="outputs", help='Output directory (overridden by config).')
    parser.add_argument('--save_csv', type=str, default="csv_outputs/", help='Output directory.')
    return parser

# Set WANDB_API_KEY in your environment (e.g. `wandb login`) rather than hardcoding it.
WANDB_CONFIG = {"WANDB_IGNORE_GLOBS": "*.patch", 
...
def main(args):
    ...
    if args.logger == 'wandb':
        ...
        logger = WandbLogger(project="HaMeR", config=vars(args))
    else:
        logger = TensorBoardLogger(save_dir=f'{args.log_dir}/log_{current_time}', name="Sign2GPT")
    
    dirpath = f'{args.output_dir}/run_{current_time}_landa_desc_{args.landa_desc}_landa_hamer_{args.landa_hamer}_lr_{args.lr}_warmup_{args.warmup}'
